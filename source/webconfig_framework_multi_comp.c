...
void multiCompBroadCastSignal_callbk(char* info, void *user_data)
{
    ...
    WbInfo(("%s : Data Received is %s , user data is %s \n",__FUNCTION__,info,(char*)user_data));
      ...
      if ( gMultiCompExecState == NULL )
      {
        ...
        if ( validResponse == 1 )
        {
          if ( atoi(token[i]) == READY_TO_RECEIVE_DATA )
                {
                    WbInfo(("READY_TO_RECEIVE_DATA, sending pthread_cond_signal\n"));
                    gReadyToReceive = 1;
                ...
                else if ( atoi(token[i])  == TIMEOUT_VALUE )
                {
                    ...
                    WbInfo(("Received timeout is %d,\n",lMultiCompExecData->timeout));
                    pthread_cond_signal(&MultiCompCond);
                }

                else if ( atoi(token[i])  == EXECUTION_SUCCESSFULLY_COMPLETED )
                {
                  WbInfo(("EXECUTION_SUCCESSFULLY_COMPLETED, sending pthread_cond_signal\n"));
                ...
                else if ( atoi(token[i]) == EXECUTION_FAILED )
                {
                      ...
                      WbInfo(("EXECUTION_FAILED, sending pthread_cond_signal\n"));
        	...
        	if ( strcmp(subdoc_name,lmultiCompRegData->multi_comp_subdoc )  == 0 )
          	{
                	...
                	if ( ret != ROLLBACK_SUCCESS )
                 	{
                     		WbError(("rollback failed for %s, return value is %d\n",subdoc_name,ret));
                 	}
          	}
...
void sendSignaltoRollBack(char* comp_name, char* subdoc_name)
{
        ...
        if ( err == ETIMEDOUT )
        {
            WbError(("%s : Component failed to respond to rollback the request\n",__FUNCTION__));   
            return;                  
        }
...
void* ExecuteMultiCompRequest_thread(void* arg)
{
       ...
       // strncpy(data,(char*)arg,sizeof(data)-1);
        ...
        while (1)
        {
            ...
            {   
                WbInfo(("%s : Queue is Empty, executing slave request \n",__FUNCTION__));
          ...
          slaveSubscribeTimeCounter = 0 , gReadyToReceive = 0 , needEventUnSubscribe = 0;
			...
			// Waiting for single component request to complete , priority is given to single component
            		...
            		while (1)
            		{
                      		...
                      		if ( isQueueEmpty() )
                      		{   
                             		WbInfo(("%s : Queue is Empty, executing multicomp request \n",__FUNCTION__));
            		...
            		WbInfo(("Execution needed in sequence %d\n",sequenceData->isExecInSequenceNeeded));
                ...
                pthread_mutex_unlock(&multiCompState_access);
                 ...
                 EventRegister(MASTER_COMP_SIGNAL_NAME);
            		 ...
            		 {
                		WbInfo(("numOfComponents in multi comp request %d\n",sequenceData->numOfComponents));
					...
					// If sequential execution is needed , wait for client reply that execution is completed , or wait till MAX timeout.
                 	...
                 	if (  sequenceData->isExecInSequenceNeeded == 0 )
                 	{
                    		...
                    		{ 
                        		...
                        		{
                              			WbInfo(("Execution completed, call post execution\n"));
                                    		if ( sequenceData->postExecSettings != NULL )
                                    		{
                                            		if ( POST_EXEC_SUCCESS == sequenceData->postExecSettings() )
                                            		{
                                                  		WbInfo(("post exec success , sending successful completion signal\n"));
                                            		}
                                    		 ...
                                    		 {
                                       			WbInfo(("post exec in NULL , sending successful completion signal\n"));
                                       			goto SENDACK;
...
void initMessageQueueMaster()
{
    ...
    pthread_t tid;
      ...
      if ( ret != 0 )
        WbError(("%s: messageQueueProcessingMultiComp pthread_create failed , ERROR : %s \n", __FUNCTION__,strerror(errno)));
...
void initMessageQueueSlave()
{
  WbInfo(("Inside FUNC %s LINE %d \n",__FUNCTION__,__LINE__));
    ...
    if ( ret != 0 )
      WbError(("%s: messageQueueProcessingMultiCompSlave pthread_create failed , ERROR : %s \n", __FUNCTION__,strerror(errno)));
...
void PushMultiCompBlobRequest (execData *exec_data )
{
    ...
    int retVal = 0 ;
  	...
  	{
        	WbError(("New version available , prcessing new Blob request\n"));
    		...
    		{
      			WbError(("calculateTimeout is NULL , using default timeout routine\n"));
